...
import PosterArt from "@/components/mobile-original/ui/PosterArt";
import MediaStatusBadge from "@/components/mobile-original/ui/MediaStatusBadge";
import EpisodeRatingFlow from "@/components/mobile-original/EpisodeRatingFlow";
import EpisodeDetail from "@/components/mobile-original/EpisodeDetail";
import ImagePickerScreen from "@/components/mobile-original/ImagePickerScreen";
import StatusMenu, { statusMenuOptions } from "@/components/mobile-original/StatusMenu";
import SeasonBanner from "@/components/mobile-original/SeasonBanner";
import SeasonRatingScreen from "@/components/mobile-original/SeasonRatingScreen";
import ShareRatingCard from "@/components/mobile-original/ShareRatingCard";
import CollectionPickerCard from "@/components/mobile-original/CollectionPickerCard";
...
export default function ShowDetailClient({ showId, show, initialSeasons, cast, videos, similar, watchProviders }) {
  ...
  useEffect(() => {
    ...
    getSeasonRatings(user.id, showId)
      .then((data) => { if (!cancelled && hydrationGeneration === hydrationGenerationRef.current && !removingRef.current) setSeasonRatings(data); })
      .catch(console.error)
  ...
  useEffect(() => {
    if (!pendingStatusSync || removingRef.current) return;
  ...
  const setEpisodeWatchCount = (seasonId, n, count) => {
    if (removingRef.current) return false;
  ...
  const selectStatus = (id) => {
    ...
    if (id === "remove") {
      ...
      Promise.allSettled([libraryWriteChainRef.current, ...Object.values(episodeWriteChainsRef.current)])
