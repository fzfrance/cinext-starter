...
export default function Page() {
  ...
  // Immediate refetch when Remove (or watchlist clear) wipes history —
  // Home already listens; Highlights must too or monthEntries stay stale
  // and the write-back effect can repopulate session cache with ghosts.
  useEffect(() => {
    const onWatchDataChanged = (event) => {
      if (user?.id && event.detail?.userId && event.detail.userId !== user.id) return;
      silentRefetchRef.current = true;
      silentMonthRefetchRef.current = true;
      setRetryToken((n) => n + 1);
    };
