import { mutateShow, resetShow, showGeneration, assertShowGeneration } from "@/lib/showMutations";
// ---------------------------------------------------------------------------
...
import { getEpisodeSkips, getShowSkipSummary, deleteAllEpisodeSkipsForShow } from "@/lib/episodeSkips";
...
import { invalidateWatchCaches } from "@/lib/sessionCaches";
...
// when it was favorited). updatedAt (also ms) is bumped by setShowStatusImpl/
// setShowFavoriteImpl on every write — Home's Continue Watching hero uses it to
// pick the most recently touched "watching" show.
const userShowsInFlight = new Map();

async function loadUserShows(userId) {
...
// Several global providers and the active page mount in the same commit and
// may ask for this identical snapshot at once. Share only the in-flight
// request; once it settles, the next call still performs a fresh no-store
// read, so mutations and later navigations cannot see a stale memory cache.
export function getUserShows(userId) {
  const existing = userShowsInFlight.get(userId);
  if (existing) return existing;

  const request = loadUserShows(userId);
  userShowsInFlight.set(userId, request);
  request.then(
    () => { if (userShowsInFlight.get(userId) === request) userShowsInFlight.delete(userId); },
    () => { if (userShowsInFlight.get(userId) === request) userShowsInFlight.delete(userId); }
  );
  return request;
}
...
async function setShowStatusImpl(userId, tmdbShowId, status, source = "unknown", { explicit } = {}) {
...
// created by an explicit status pick (setShowStatusImpl), never as a side
...
async function setShowFavoriteImpl(userId, tmdbShowId, favorite, source = "unknown") {
...
// underneath it.
async function removeUserShowImpl(userId, tmdbShowId, source = "unknown") {
  // Normalize route IDs and validate them before any destructive query.
  const showId = Number(tmdbShowId);
  if (!Number.isFinite(showId)) throw new Error(`Invalid tmdbShowId: ${tmdbShowId}`);

  const { data: existing } = await supabase.from("user_shows").select("status").eq("user_id", userId).eq("tmdb_show_id", showId).maybeSingle();
  logLibraryWrite("remove", { source, tmdbShowId: showId, oldStatus: existing?.status ?? null, newStatus: null, note: "cascading: episode_watches + season_reviews + season_ratings + user_shows" });

  // Watch history / skips are required. A prior allSettled here let those
  // deletes fail while still dropping user_shows — library looked removed
  // but Highlights / Continue Watching kept the episodes.
  await deleteAllEpisodeWatchesForShow(userId, showId);
  await deleteAllEpisodeSkipsForShow(userId, showId);

  // A full reset must report dependent cleanup failures, never hide them.
  await deleteAllSeasonReviewsForShow(userId, showId);
  await deleteAllSeasonRatingsForShow(userId, showId);

  const { error } = await supabase.from("user_shows").delete().eq("user_id", userId).eq("tmdb_show_id", showId);
  if (error) throw error;
  const { count, error: verifyError } = await supabase.from("user_shows")
    .select("id", { count: "exact", head: true }).eq("user_id", userId).eq("tmdb_show_id", showId);
  if (verifyError) throw verifyError;
  if (count !== 0) throw new Error("Show removal could not be verified");
  // The reset wrapper invalidates all consumers on success or partial failure.
}
...
// must not double every episode's watch count. Already-Skipped episodes
// are left alone too, not overwritten with a forced watch — Skipped
// already counts as resolved (see lib/statusResolver.js), and a bulk
// "mark completed" action must not silently turn a deliberate skip into
// a watch the user never actually did. /api/shows/[id]/aired-episodes
// already excludes Specials (season 0) at the source, so this never
// touches those either.
export async function markShowCompleted(userId, tmdbShowId, source = "unknown") {
  const generation = showGeneration(userId, tmdbShowId);
  const [alreadyWatched, alreadySkipped, res] = await Promise.all([
    getEpisodeWatches(userId, tmdbShowId),
    getEpisodeSkips(userId, tmdbShowId),
    fetch(`/api/shows/${tmdbShowId}/aired-episodes`).then((r) => r.json()),
  ]);
  const toMark = (res.episodes ?? [])
    .filter((e) => !alreadyWatched[`${e.season}-${e.episode}`] && !alreadySkipped.has(`${e.season}-${e.episode}`))
    .map((e) => ({ seasonNumber: e.season, episodeNumber: e.episode }));

  assertShowGeneration(userId, tmdbShowId, generation);
  await Promise.all([
...
export function setWatchlistAndClearProgress(userId, tmdbShowId, source = "unknown") {
  return resetShow(userId, tmdbShowId, async () => {
    try {
      await deleteAllEpisodeWatchesForShow(userId, tmdbShowId);
      await deleteAllEpisodeSkipsForShow(userId, tmdbShowId);
      await setShowStatusImpl(userId, tmdbShowId, "watchlist", source, { explicit: true });
    } finally {
      userShowsInFlight.delete(userId);
      invalidateWatchCaches(userId);
    }
  });
}
...
// Same reconciliation ShowDetailClient's own pendingStatusSync effect
// already does after every mark/unmark on that page — extracted here so
// any OTHER place that changes a show's watched-episode count can trigger
// the same fix-up. Episode Detail (app/show/[id]/episode/[season]/[ep] —
// the page Highlights' Watch History calendar opens into) used to call
// syncEpisodeWatchCount directly with no reconciliation at all: unmarking
// an episode there correctly deleted the episode_watches row, but
// user_shows.status/the implicit row's existence never got touched, so
// the show kept reading "Watching" (and staying in Home's In Progress)
// forever with zero actual watched episodes — exactly the "unmarked in
// Episode Detail but Show Detail still says Watching" bug this closes.
//
// Same explicit-trigger-only rule as everywhere else in this file: only
// ever call this right after a real watch-count mutation the user just
// made (mark/unmark/rewatch), never from a page-load/mount effect.
//
// Four cases, identical to ShowDetailClient's own (see that effect's
// comment for the full reasoning):
// 1. Implicit row (statusExplicit false), watched count now 0 — nothing
//    justifies the row anymore, revert it out of the library entirely.
// 2. No row yet, but this watch action gives it a resolvable status
//    (e.g. the very first episode ever marked for this show) — create it
//    implicitly.
// 3. Row exists, statusExplicit true — do nothing; an explicit pick
//    survives regardless of watched count (resolveShowStatus already
//    falls back to it live).
// 4. Row exists, implicit, watched count > 0 — keep its stored status
//    current with the live-resolved one.
export async function reconcileShowStatusAfterWatchChange(userId, tmdbShowId, source = "unknown") {
  const generation = showGeneration(userId, tmdbShowId);
  const [existing, summaries, skipSummaries] = await Promise.all([
    getUserShow(userId, tmdbShowId),
    getShowWatchSummary(userId, [tmdbShowId]),
    getShowSkipSummary(userId, [tmdbShowId]),
  ]);

  const watchedKeys = new Set(summaries[tmdbShowId]?.watchedKeys ?? []);
  const skippedKeys = new Set(skipSummaries[tmdbShowId]?.skippedKeys ?? []);
  // Already Specials-excluded — /api/shows/[id]/aired-episodes filters
  // season 0 out at the source (see that route's own comment).
  const res = await fetch(`/api/shows/${tmdbShowId}/aired-episodes`);
  const { episodes } = await res.json();
  const releasedEpisodes = episodes.length;
  const watchedReleasedEpisodes = episodes.filter((e) => watchedKeys.has(`${e.season}-${e.episode}`)).length;
  const resolvedReleasedEpisodes = episodes.filter((e) => {
    const key = `${e.season}-${e.episode}`;
    return watchedKeys.has(key) || skippedKeys.has(key);
  }).length;

  assertShowGeneration(userId, tmdbShowId, generation);
  if (existing && existing.statusExplicit === false && resolvedReleasedEpisodes === 0) {
    await removeImplicitLibraryRow(userId, tmdbShowId, source);
    return;
  }

  const resolved = resolveShowStatus({
    explicitStatus: existing?.status ?? null,
    watchedReleasedEpisodes,
    releasedEpisodes,
    resolvedReleasedEpisodes,
  });

  if (!existing) {
    if (resolved) await setShowStatus(userId, tmdbShowId, resolved, source, { explicit: false });
    return;
  }

  if (existing.statusExplicit) return;

  if (resolved && resolved !== existing.status) {
    await setShowStatus(userId, tmdbShowId, resolved, source);
  }
}

export function setShowStatus(...args) {
  return mutateShow(args[0], args[1], () => setShowStatusImpl(...args));
}

export function setShowFavorite(...args) {
  return mutateShow(args[0], args[1], () => setShowFavoriteImpl(...args));
}

export function removeUserShow(userId, showId, source = "unknown") {
  return resetShow(userId, showId, async () => {
    try { return await removeUserShowImpl(userId, showId, source); }
    finally { userShowsInFlight.delete(userId); invalidateWatchCaches(userId); }
  });
}
