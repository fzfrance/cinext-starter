...
export async function removeUserShow(userId, tmdbShowId, source = "unknown") {
  ...
  // Keep the destructive Remove action from being blocked by an optional
  // cleanup table (for example an older account without season_ratings
  // permissions). Watch history and the library row must still be removed;
  // cleanup failures are logged and can be retried independently.
  const cleanupResults = await Promise.allSettled([
    deleteAllEpisodeWatchesForShow(userId, tmdbShowId),
    deleteAllEpisodeSkipsForShow(userId, tmdbShowId),
    deleteAllSeasonReviewsForShow(userId, tmdbShowId),
    deleteAllSeasonRatingsForShow(userId, tmdbShowId),
  ]);
  cleanupResults.forEach((result) => {
    if (result.status === "rejected") console.error("[library-remove] cleanup failed", result.reason);
  });

  const { error } = await supabase.from("user_shows").delete().eq("user_id", userId).eq("tmdb_show_id", tmdbShowId);
  if (error) throw error;
  // Drop Home/Highlights session caches + notify open tabs so Continue
  // Watching can't keep showing a show whose history was just wiped.
