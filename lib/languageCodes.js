...
// Prefer a native-script person name when that language is readable
// (e.g. Korean actor → 전여빈 if "ko" is readable). Falls back to the
// default/localized Latin name from the person detail fetch otherwise.
...
export function resolvePersonName(person, readableLanguages = []) {
  ...
  const byLang = person?.namesByLang;
  if (!byLang || !readableLanguages?.length) return fallback;
