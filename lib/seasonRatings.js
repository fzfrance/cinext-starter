...
export async function getAllSeasonRatingsForUser(userId) {
  const { data, error } = await supabase
    .from("season_ratings")
    .select("show_id, season_number, rating, mood, favorite_character_id, favorite_character_name, review_text, created_at, updated_at")
    .eq("user_id", userId);
...
export async function getSeasonRatings(userId, showId) {
  const { data, error } = await supabase
    .from("season_ratings")
    .select("season_number, rating, mood, favorite_character_id, favorite_character_name, review_text, share_id, updated_at")
    .eq("user_id", userId)
    .eq("show_id", showId);
  if (error) throw error;
...
export async function getSeasonRating(userId, showId, seasonNumber) {
  ...
  return {
    rating: data.rating,
    mood: data.mood,
    characterId: data.favorite_character_id,
...
export async function saveSeasonRating(userId, showId, seasonNumber, { rating, mood, characterId, characterName, text }) {
...
export async function getSeasonRatingByShareId(shareId) {
  const { data, error } = await supabase
    .from("season_ratings")
    .select("show_id, season_number, rating, mood, favorite_character_id, favorite_character_name, review_text, updated_at")
    .eq("share_id", shareId)
    .maybeSingle();
  if (error) throw error;
  if (!data) return null;
