...
export function useAppLanguageBoot() {
  ...
  useEffect(() => {
    // Sync cookie ← localStorage immediately so /api/search and RSC TMDB
    // fetches don't still see a stale App Language (e.g. zh synopsis while
    // the UI chrome already reads as English from localStorage).
  ...
  useEffect(() => {
    ...
    getProfile(user.id)
      .then((profile) => {
        ...
        // Profile is source of truth — reload so server-rendered synopsis /
        // episodes match the saved App Language (not Readable Languages).
