...
/** Drama / Comedy / Action — skip unless preference is unusually strong. */
...
function isBroadGenre(genreId, label) {
  if (BROAD_GENRE_IDS.has(Number(genreId))) return true;
  return /^(drama|comedy|action)\b/i.test(String(label || ""));
}
...
/**
 * Strict Explore personalization — only strong Because / More Like / genre
 * (and rare natural language shelves). Returns 0–3 rows; never forced.
 */
export function assembleExploreSections(scored, profile, { genreNames = new Map() } = {}) {
  ...
  const candidates = [];
  const genreLabel = (id) => displayGenreLabel(
    genreNames.get(Number(id)) || genreNames.get(String(id)) || null
  );

  // --- Because You Watched / More Like — strong single-title relationship only
  ...
  if (seed && (seed.weight ?? 0) >= 4) {
    ...
    if (related.length >= 5 && avg >= 0.45) {
      candidates.push(packSection({
        ...
        kind: "because",
        items: related,
        evidence: 6 + Math.min(4, seed.weight || 0) + avg,
      }));
      candidates.push(packSection({
  ...
  // --- Because You Like [Genre] — distinctive genres, strong evidence
  const genreEntries = [...(profile.watchedGenreCounts ?? new Map()).entries()]
    .sort((a, b) => b[1] - a[1]);

  for (const [genreId, count] of genreEntries) {
    if (count < MIN_GENRE_LIKE_WATCHED) continue;
    const label = genreLabel(genreId);
    if (!label) continue;
    // Broad genres need unusually strong preference (6+)
    if (isBroadGenre(genreId, label) && count < 6) continue;

    const fromGenre = eligible
      .filter((item) => (item.genreIds ?? []).map(Number).includes(Number(genreId)))
      .filter((item) => (item.score ?? 0) >= 0.4)
      .slice(0, 20);
    const section = packSection({
      id: `like-genre-${genreId}`,
      title: `Because You Like ${label}`,
      kind: "likeGenre",
      items: fromGenre,
      evidence: 5 + Math.min(5, count),
    });
    if (section) {
  ...
  // Strongest 1–3 only; skip weak/awkward (already filtered). Prefer variety.
  ...
  for (const row of picked) {
    ...
    // Don't show both Because and More Like for the same seed — pick the stronger one
    if ((row.kind === "because" || row.kind === "moreLike") && (usedKinds.has("because") || usedKinds.has("moreLike"))) {
      continue;
    }
