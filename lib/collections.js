...
// Returns [{ id, name, description, coverStyle, createdAt, lastAddedAt, showIds, movieIds }]
// for the user's whole collections list, most-recent membership first
// (falls back to created_at for empty collections). showIds/movieIds are
// themselves newest-added-first so pickers and box-set previews surface
// recent titles. Movie-read errors are surfaced instead of silently
// turning a populated movie collection into an empty one in every UI.
export async function getCollections(userId) {
  ...
  const { data: movies, error: moviesError } = await supabase
    ...
    .select("collection_id, tmdb_movie_id, added_at")
    .in("collection_id", collectionIds);
  if (moviesError) throw moviesError;

  const byAddedDesc = (a, b) => new Date(b.added_at || 0) - new Date(a.added_at || 0);

  return collections
    .map((c) => {
      const collectionShows = shows.filter((s) => s.collection_id === c.id).sort(byAddedDesc);
      const collectionMovies = movies.filter((m) => m.collection_id === c.id).sort(byAddedDesc);
      const lastShowAt = collectionShows[0]?.added_at ?? null;
      const lastMovieAt = collectionMovies[0]?.added_at ?? null;
      const lastAddedAt = [lastShowAt, lastMovieAt, c.created_at]
        .filter(Boolean)
        .sort((a, b) => new Date(b) - new Date(a))[0] ?? c.created_at;
      return {
        id: c.id,
        name: c.name,
        ...
        // Newest membership first — pickers / box-set previews read from the
        // front of these arrays.
        showIds: collectionShows.map((s) => s.tmdb_show_id),
        movieIds: collectionMovies.map((m) => m.tmdb_movie_id),
      };
    })
    .sort((a, b) => new Date(b.lastAddedAt || 0) - new Date(a.lastAddedAt || 0));
}
