...
export async function getMyRatingsForUser(userId) {
  ...
  const entries = [
    ...manualRows.map((r) => ({ ...r, mediaType: "tv", isAuto: false, activityAt: r.savedAt })),
    ...autoEligible.map((r) => ({ showId: r.showId, seasonNumber: r.seasonNumber, mediaType: "tv", rating: r.avg10, mood: null, characterName: null, text: "", isAuto: true, activityAt: r.lastRatedAt })),
    ...movieRows.map((r) => ({ movieId: r.movieId, seasonNumber: null, mediaType: "movie", rating: r.rating, mood: r.mood, characterName: r.characterName, text: r.text, isAuto: false, activityAt: r.savedAt })),
