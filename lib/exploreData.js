...
import {
  EXPLORE_GENRE_RAILS,
  GENRE_RAIL_INITIAL_PAGES,
  fetchGenreRailPages,
} from "@/lib/exploreGenreRails";
...
async function fetchGenreRails() {
  const rails = await Promise.all(
    EXPLORE_GENRE_RAILS.map(async (rail) => {
      const { items, hasMore } = await fetchGenreRailPages(rail, 1, GENRE_RAIL_INITIAL_PAGES);
      return {
        name: rail.name,
        items,
        nextPage: GENRE_RAIL_INITIAL_PAGES + 1,
        hasMore,
      };
    })
  );
