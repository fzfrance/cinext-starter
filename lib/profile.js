...
// themePreference, accentColor, handle, appLanguage? } or null if this user
// has never saved a profile yet. readableLanguages defaults to ["en"] even
// for an existing row — the column itself defaults to array['en'] at the DB
// level, but this covers a row saved before that column existed too. Same
// reasoning for themePreference/accentColor defaulting to 'dark'/DEFAULT_ACCENT.
// appLanguage is omitted when the DB column is missing.
const profilesInFlight = new Map();

async function loadProfile(userId) {
  const withAppLanguage = "display_name, bio, avatar_url, background_url, readable_languages, theme_preference, accent_color, handle, app_language";
  const withoutAppLanguage = "display_name, bio, avatar_url, background_url, readable_languages, theme_preference, accent_color, handle";

  let { data, error } = await supabase
    .from("profiles")
    .select(withAppLanguage)
    .eq("user_id", userId)
    .maybeSingle();

  // Older DBs may not have app_language yet — retry without it.
  if (error && /app_language/i.test(error.message || "")) {
    ({ data, error } = await supabase
      .from("profiles")
      .select(withoutAppLanguage)
      .eq("user_id", userId)
      .maybeSingle());
  }
  ...
  return data
    ? {
        ...
        handle: data.handle ?? "",
        appLanguage: data.app_language || undefined,
      }
    : null;
}
...
export async function upsertProfile(userId, { displayName, bio, avatarUrl, backgroundUrl, readableLanguages, themePreference, accentColor, handle, appLanguage }) {
  ...
  if (appLanguage !== undefined) payload.app_language = appLanguage;
  ...
  if (error) {
    ...
    // app_language may not exist on older DBs yet — retry without it so
    // localStorage-backed App Language still works on desktop web.
    if (appLanguage !== undefined && /app_language/i.test(error.message || "")) {
      delete payload.app_language;
      const retry = await supabase.from("profiles").upsert(payload, { onConflict: "user_id" });
      if (retry.error) {
        if (retry.error.code === "23505" && retry.error.message?.includes("handle")) {
          throw new Error("That handle is already taken.");
        }
        throw retry.error;
      }
