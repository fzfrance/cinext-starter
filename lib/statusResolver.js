...
// resolvedReleasedEpisodes: watched OR skipped, Specials (season 0)
// excluded — defaults to watchedReleasedEpisodes so every caller that
// doesn't yet fetch skip data (most of them — see ShowDetailClient.jsx
// for the one that does) keeps behaving exactly as before. releasedEpisodes/
// watchedReleasedEpisodes are also expected to already exclude Specials
// (every caller of this function computes them that way now — see
// app/api/shows/library-detail's own resolveShow) so a show whose only
// unwatched episodes are Specials still resolves to "completed" here,
// without this function needing to know about seasons/episodes itself.
export function resolveShowStatus({ explicitStatus, watchedReleasedEpisodes = 0, releasedEpisodes = 0, resolvedReleasedEpisodes = watchedReleasedEpisodes }) {
  if (explicitStatus === "paused" || explicitStatus === "drop" || explicitStatus === "completed") {
    return explicitStatus;
  }
  if (releasedEpisodes > 0 && resolvedReleasedEpisodes === releasedEpisodes) {
    return "completed";
  }
  if (resolvedReleasedEpisodes > 0) {
    return "watching";
  }
