...
// watchDateSource } } — the date fields describe the MOST RECENT watch
// event for that episode, same "most recent watched_at wins" definition
// getLatestWatchDate uses for a single episode, just computed for every
// episode of the show in one query (ShowDetailClient's own per-episode
// row needs this for all of them at once, not one at a time) so it can
// show a "Watched: {date}" line the same way EpisodeDetail.jsx already
// does. Ascending watched_at order means each key's fields are simply
// overwritten as rows are iterated — the last write for a given key is
// always its most recent watch, no extra comparison needed.
export async function getEpisodeWatches(userId, tmdbShowId) {
  const { data, error } = await supabase
    .from("episode_watches")
    .select("season_number, episode_number, rating, watched_at, watch_date_precision, watched_on, watched_year, watched_month, watch_date_source")
    .eq("user_id", userId)
  ...
  for (const row of data) {
    ...
    const entry = byEpisode[key] ?? { watchCount: 0, rating: null, watchDatePrecision: null, watchedOn: null, watchedYear: null, watchedMonth: null, watchDateSource: null };
    entry.watchCount += 1;
    if (row.rating != null) entry.rating = row.rating;
    entry.watchDatePrecision = row.watch_date_precision;
    entry.watchedOn = row.watched_on;
    entry.watchedYear = row.watched_year;
    entry.watchedMonth = row.watched_month;
    entry.watchDateSource = row.watch_date_source;
    byEpisode[key] = entry;
  }
  return byEpisode;
}
