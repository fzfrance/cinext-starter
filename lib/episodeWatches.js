import { mutateShow } from "@/lib/showMutations";
// ---------------------------------------------------------------------------
...
export async function getRatedEpisodesAcrossShows(userId) {
  ...
  for (let from = 0; ; from += pageSize) {
    const { data: page, error } = await supabase
      .from("episode_watches")
      .select("id, tmdb_show_id, season_number, episode_number, rating, watched_at")
      .eq("user_id", userId)
      .not("rating", "is", null)
...
// write time, see addEpisodeWatchesImpl/setWatchDate below), so one plain
...
export async function getWatchedEpisodesForYear(userId, year) {
  ...
  const ranges = [
    ...
    // Only pre-precision legacy rows may fall back to watched_at. Rows
    // explicitly marked "Don't remember" also have watched_year=null, but
    // their watched_at is merely when Cinext recorded/imported the status;
    // treating that insertion timestamp as history placed dozens of old
    // completed shows into the current Time Machine year.
    (query) => query.is("watched_year", null)
      .or("watch_date_precision.is.null,watch_date_precision.neq.unknown")
...
export async function getWatchedYears(userId) {
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    const { data: page, error } = await supabase
      .from("episode_watches")
      .select("watched_year, watched_at, watch_date_precision")
      .eq("user_id", userId)
      .eq("hidden_from_highlights", false)
      .range(from, from + PAGE_SIZE - 1);
    if (error) throw error;
  ...
  const years = new Set();
  for (const row of data) {
    if (row.watched_year != null) {
      years.add(Number(row.watched_year));
      continue;
    }
    // Legacy rows may have only watched_at. Do not infer a year for an
    // explicit "Don't remember" entry, but preserve ordinary pre-migration
    // rows whose precision was null or day-based.
    if (row.watch_date_precision === "unknown" || !row.watched_at) continue;
    const legacyYear = Number(new Date(row.watched_at).toLocaleString("en-US", {
      timeZone: "Asia/Bangkok", year: "numeric",
    }));
    if (Number.isFinite(legacyYear)) years.add(legacyYear);
  }
...
export async function deleteAllEpisodeWatchesForShow(userId, tmdbShowId) {
  ...
  // PostgREST can return success with 0 rows deleted when filters don't
  // match or DELETE policies deny the rows. Verify instead of assuming.
  ...
  if (count !== 0) {
    throw new Error(`episode_watches wipe incomplete for show ${showId}: ${count} row(s) remain`);
  }
}
...
async function addEpisodeWatchesImpl(userId, tmdbShowId, episodes) {
...
async function clearEpisodeWatchesImpl(userId, tmdbShowId, seasonNumber, episodeNumber) {
  const showId = Number(tmdbShowId);
  const { error } = await supabase
    .from("episode_watches")
    .delete()
...
async function syncEpisodeWatchCountImpl(userId, tmdbShowId, seasonNumber, episodeNumber, count) {
  await clearEpisodeWatchesImpl(userId, tmdbShowId, seasonNumber, episodeNumber);
  if (count > 0) {
    await addEpisodeWatchesImpl(userId, tmdbShowId, Array.from({ length: count }, () => ({ seasonNumber, episodeNumber })));
  }
}
...
// targeting rateLatestWatchImpl below already uses for star ratings in that
...
async function rateLatestWatchImpl(userId, tmdbShowId, seasonNumber, episodeNumber, rating) {
  const { data, error } = await supabase
    .from("episode_watches")
    .select("id")
    .eq("user_id", userId)
...
// "the most recent watch" like rateLatestWatchImpl above. Highlights' Watch
...
export function addEpisodeWatches(...args) {
  return mutateShow(args[0], args[1], () => addEpisodeWatchesImpl(...args));
}

export function clearEpisodeWatches(...args) {
  return mutateShow(args[0], args[1], () => clearEpisodeWatchesImpl(...args));
}

export function syncEpisodeWatchCount(...args) {
  return mutateShow(args[0], args[1], () => syncEpisodeWatchCountImpl(...args));
}

export function rateLatestWatch(...args) {
  return mutateShow(args[0], args[1], () => rateLatestWatchImpl(...args));
}
