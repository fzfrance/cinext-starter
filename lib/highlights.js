...
// Ranks shows by the user's own rating first (highest first) — a show
// they rated 9/10 outranks one they merely binged more of. Only shows
// with NO resolvable rating at all fall back to the old purely time-based
// ordering: total minutes watched (1), episode count as a tie-break (2),
// then most recent activity as the final tie-break (3). Rated shows
// always sort ahead of unrated ones, regardless of hours.
//
// ratingByShowSeason (Map<"showId-seasonNumber", rating>, see
// lib/myRatings.js's resolveSeasonRatings) is optional and defaults to
// empty — callers that don't pass one (e.g. the year-only Top Shows view,
// which has no per-episode entries to resolve season ratings against)
// keep today's pure time-based behavior unchanged.
//
// A show can have multiple seasons touched within the ranked period —
// reduced to one rating by picking whichever season had the most
// watched-entries this period (ties broken by the higher rating), the
// simplest rule that reflects "what the user was actually watching,"
// without averaging across seasons of wildly different rated status.
export function computeTopShows(entries, limit = 3, ratingByShowSeason = new Map()) {
  ...
  for (const e of entries) {
    ...
    bucket.seasonCounts.set(e.season, (bucket.seasonCounts.get(e.season) ?? 0) + 1);
  ...
  return [...byShow.values()]
    .map((bucket) => {
      let rating = null;
      let bestCount = -1;
      for (const [season, count] of bucket.seasonCounts) {
        const r = ratingByShowSeason.get(`${bucket.showId}-${season}`);
        if (r == null) continue;
        if (count > bestCount || (count === bestCount && r > rating)) { bestCount = count; rating = r; }
      }
      const { seasonCounts, ...rest } = bucket;
      return { ...rest, rating };
    })
    .sort((a, b) => {
      ...
      if (aRated) return b.rating - a.rating || b.totalMinutes - a.totalMinutes;
      return (
...
// Movies analog of computeTopShows — one movie_ratings row per movie (no
// season reduction needed, see lib/movieRatings.js), so the rating lookup
// is a direct Map get. Unrated movies fall back to runtime minutes
// (there's no "episode count" equivalent for a movie).
export function computeTopMovies(entries, limit = 5, ratingByMovie = new Map()) {
  return entries
    .map((e) => ({ ...e, rating: ratingByMovie.get(e.movieId) ?? null }))
    .sort((a, b) => {
      const aRated = a.rating != null;
      const bRated = b.rating != null;
      if (aRated !== bRated) return aRated ? -1 : 1;
