...
import { APP_LANGUAGE_STORAGE_KEY, DEFAULT_APP_LANGUAGE, normalizeAppLanguage, toTmdbLanguage, TMDB_LANGUAGE_TAGS } from "@/lib/languageCodes";
import { DEFAULT_WATCH_REGION, expandWatchProviderIds } from "@/lib/discoverFilters";
 ...
 *  queries still hit even when App Language is something else. Also
 *  includes the user's Readable Languages so Latin queries still gather
 *  localized title/name variants for *ranking* (searchTitles). Display
 *  copy (title/overview/name) always comes from App Language — see
 *  mergeSearchPages. */
function searchLanguageTagsForQuery(query, readableLanguages = []) {
  ...
  for (const code of readableLanguages) {
    const tag = TMDB_LANGUAGE_TAGS[code];
    if (tag) tags.add(tag);
  }
  return [...tags];
}
...
async function mergeSearchPages(path, query, page, languageTags) {
  ...
  // Display passes: artwork + interface copy (title defaults, synopsis).
  // Readable / script tags are match-bait only (searchTitles) — never synopsis.
  // Product rule: show/movie *titles* may switch via resolveTitle(Readable);
  // synopsis / cast copy always follow App Language.
  const tags = [...new Set([null, "en-US", appTag, ...languageTags.filter(Boolean)])];
  const pages = await Promise.all(
    tags.map(async (language) => ({
      language,
      data: await tmdbFetch(path, { query, page, include_adult: false, language }, 600).catch(() => ({ results: [] })),
    }))
  );

  const byId = new Map();
  for (const { language, data } of pages) {
    const isDisplayPass = language === null || language === "en-US" || language === appTag;
    const isAppPass = language === appTag;
    const isEnPass = language === "en-US" || language === null;
    for (const item of data.results ?? []) {
      const incomingTitles = [item.title, item.name, item.original_title, item.original_name].filter(Boolean);
      const prev = byId.get(item.id);
      if (!prev) {
        byId.set(item.id, {
          ...item,
          // Readable-only first hits must not lock a Chinese/etc. synopsis
          // under an English display title (resolveTitle can restore the
          // original name while overview would stay translated).
          overview: isDisplayPass ? (item.overview ?? "") : "",
          searchTitles: incomingTitles,
          _overviewSource: isDisplayPass ? (isAppPass ? "app" : isEnPass ? "en" : "display") : null,
        });
        continue;
      }
      ...
      if (isDisplayPass) {
        // Prefer App Language synopsis, then English/null, never a weaker blank.
        let overview = prev.overview;
        let overviewSource = prev._overviewSource;
        if (item.overview) {
          if (isAppPass) {
            overview = item.overview;
            overviewSource = "app";
          } else if (overviewSource !== "app" && (isEnPass || !overview)) {
            overview = item.overview;
            overviewSource = isEnPass ? "en" : "display";
          }
        }
        ...
        byId.set(item.id, {
          ...prev,
          ...item,
          // Stable artwork from the earliest pass; fill gaps only.
          poster_path: prev.poster_path || item.poster_path,
          backdrop_path: prev.backdrop_path || item.backdrop_path,
          profile_path: prev.profile_path || item.profile_path,
          original_name: prev.original_name || item.original_name || null,
          original_title: prev.original_title || item.original_title || null,
          overview,
      ...
      // Readable / script locale — match bait only. Never replace the
      // App Language title, name, or synopsis with another translation.
      byId.set(item.id, {
        ...prev,
        original_name: prev.original_name || item.original_name || null,
        original_title: prev.original_title || item.original_title || null,
  ...
  const results = [...byId.values()].map(({ _overviewSource, ...item }) => item);
  return { results };
}
...
export function searchShows(query, page = 1, readableLanguages = []) {
  return mergeSearchPages("/search/tv", query, page, searchLanguageTagsForQuery(query, readableLanguages));
}
...
// Genre ± original-language discovery for taste-matched For You rails.
export function discoverShowsForTaste({ genreId, language, page = 1 } = {}) {
  const params = { page, sort_by: "popularity.desc", "vote_count.gte": 40 };
  if (genreId) params.with_genres = genreId;
  if (language) params.with_original_language = language;
  return listWithStablePosters("/discover/tv", params);
}
...
// (e.g. "TH") and a provider ID. Look up provider IDs via /watch/providers/tv.
// Some brands use different ids per region (Disney+ 337 vs 122) — expand aliases.
export function discoverShowsByPlatform(providerId, region = DEFAULT_WATCH_REGION, page = 1) {
  const providers = expandWatchProviderIds([providerId]).join("|") || String(providerId);
  return listWithStablePosters("/discover/tv", {
...
export function getWatchProvidersList(region = DEFAULT_WATCH_REGION) {
...
export async function discoverLibrary({ genre, yearFrom, yearTo, platforms = [], languages = [], page = 1, region = DEFAULT_WATCH_REGION } = {}) {
  ...
  if (platforms.length > 0) {
    const expanded = expandWatchProviderIds(platforms);
    baseParams.with_watch_providers = (expanded.length ? expanded : platforms).join("|");
...
// images + tagged_images + external_ids feed the desktop person hero and
// social row (prefer true landscape tagged stills when TMDB has them).
export function getPersonDetails(personId) {
  return tmdbFetch(
    `/person/${personId}`,
    { append_to_response: "combined_credits,images,tagged_images,external_ids,translations" },
    DETAIL_REVALIDATE_SECONDS
  );
}

// Keep the same TMDB primary portrait used by cast cards. Resolution alone
// can select a full-body upload with unsuitable framing for a profile.
export function pickBestPersonProfile(person) {
  if (person?.profile_path) return person.profile_path;
  const profiles = (person?.images?.profiles ?? []).filter((img) => img?.file_path);
...
// Desktop hero artwork: prefer a real landscape tagged still (matches the
// cinematic full-bleed ref). Fall back to the best vertical profile, which
// the UI treats as a center-right composition over a blurred fill.
export function pickPersonHeroArt(person) {
  const profile = pickBestPersonProfile(person);
  if (profile) return { path: profile, kind: "portrait" };
  const tagged = person?.tagged_images?.results ?? [];
  const landscape = tagged
    .filter((img) => img?.file_path && (img.aspect_ratio ?? 0) >= 1.35)
    .sort(
      (a, b) =>
        (b.vote_count ?? 0) - (a.vote_count ?? 0)
        || (b.width ?? 0) - (a.width ?? 0)
  ...
  if (landscape[0]) {
    return { path: landscape[0].file_path, kind: "landscape" };
  }

  return { path: null, kind: "none" };
}

export function buildPersonSocialLinks(externalIds = {}, homepage = null) {
  const links = [];
  const ext = externalIds || {};
  if (ext.instagram_id) {
    links.push({ id: "instagram", label: "Instagram", href: `https://www.instagram.com/${ext.instagram_id}` });
  }
  if (ext.twitter_id) {
    links.push({ id: "twitter", label: "X", href: `https://x.com/${ext.twitter_id}` });
  }
  if (ext.facebook_id) {
    links.push({ id: "facebook", label: "Facebook", href: `https://www.facebook.com/${ext.facebook_id}` });
  }
  if (ext.tiktok_id) {
    links.push({ id: "tiktok", label: "TikTok", href: `https://www.tiktok.com/@${ext.tiktok_id}` });
  }
  if (ext.youtube_id) {
    links.push({ id: "youtube", label: "YouTube", href: `https://www.youtube.com/${ext.youtube_id}` });
  }
  if (ext.imdb_id) {
    links.push({ id: "imdb", label: "IMDb", href: `https://www.imdb.com/name/${ext.imdb_id}` });
  }
  if (homepage) {
    links.push({ id: "homepage", label: "Website", href: homepage });
  }
  return links;
}
...
export function searchMovies(query, page = 1, readableLanguages = []) {
  return mergeSearchPages("/search/movie", query, page, searchLanguageTagsForQuery(query, readableLanguages));
}
...
// ranking for free.
export function searchMulti(query, page = 1, readableLanguages = []) {
  return mergeSearchPages("/search/multi", query, page, searchLanguageTagsForQuery(query, readableLanguages));
}

export function searchPerson(query, page = 1, readableLanguages = []) {
  return mergeSearchPages("/search/person", query, page, searchLanguageTagsForQuery(query, readableLanguages));
}
...
export function discoverMoviesForTaste({ genreId, language, page = 1 } = {}) {
  const params = { page, sort_by: "popularity.desc", "vote_count.gte": 40 };
  if (genreId) params.with_genres = genreId;
  if (language) params.with_original_language = language;
  return listWithStablePosters("/discover/movie", params);
}
...
export function discoverMoviesByPlatform(providerId, region = DEFAULT_WATCH_REGION, page = 1) {
  const providers = expandWatchProviderIds([providerId]).join("|") || String(providerId);
...
export function getWatchProvidersListMovie(region = DEFAULT_WATCH_REGION) {
...
// of first_air_date.
export async function discoverMovieLibrary({ genre, yearFrom, yearTo, platforms = [], languages = [], page = 1, region = DEFAULT_WATCH_REGION } = {}) {
  const baseParams = { page, sort_by: "popularity.desc" };
  if (genre) baseParams.with_genres = genre;
...
export function getMovieDetails(movieId) {
  ...
  // keywords feed the rule-based recommendation taste profile.
  return withStableArtwork(`/movie/${movieId}`, "credits,videos,release_dates,keywords");
}
...
async function youtubeKeyIsPlayable(key) {
  if (!key) return false;
  try {
    const url = `https://www.youtube.com/oembed?url=${encodeURIComponent(`https://www.youtube.com/watch?v=${key}`)}&format=json`;
    const res = await fetch(url, {
      // Cache a day — removed uploads stay gone; new ones rarely replace the same key.
      next: { revalidate: 86400 },
      signal: AbortSignal.timeout(3500),
    });
    return res.ok;
  } catch {
    return null; // network/timeout — unknown, not a confirmed dead link
  }
}

function shapeMediaVideo(v) {
  return { key: v.key, name: v.name ?? "", type: v.type ?? "Trailer" };
}

/**
 * Trailer & More: only YouTube Trailers/Teasers that still resolve.
 * TMDB often keeps Clip / Featurette / BTS keys after the upload was
 * removed or region-locked — those become dead "unavailable" tiles.
 */
export async function pickPlayableVideos(videos, { limit = 8 } = {}) {
  const youtube = (videos ?? []).filter((v) => v?.site === "YouTube" && v?.key);
  const ordered = [
    ...youtube.filter((v) => v.type === "Trailer").sort((a, b) => (b.official ? 1 : 0) - (a.official ? 1 : 0)),
  ...
  for (const v of ordered) {
    ...
    seen.add(v.key);
    candidates.push(v);
    if (candidates.length >= Math.max(limit * 2, 8)) break;
  }
  if (candidates.length === 0) return [];

  const checks = await Promise.all(
    candidates.map(async (v) => ({ v, ok: await youtubeKeyIsPlayable(v.key) }))
  );
  const confirmed = checks.filter((c) => c.ok === true).map((c) => c.v);
  if (confirmed.length > 0) {
    return confirmed.slice(0, limit).map(shapeMediaVideo);
  }
  // If oEmbed couldn't be reached at all, still never attach Clip/etc. —
  // fall back to Trailer keys only (unverified).
  const allUnknown = checks.every((c) => c.ok == null);
  if (allUnknown) {
    return candidates.filter((v) => v.type === "Trailer").slice(0, limit).map(shapeMediaVideo);
  }
  return [];
}

export async function pickPlayableTrailerKey(videos) {
  const list = await pickPlayableVideos(videos, { limit: 1 });
