...
async function tmdbFetch(path, params = {}) {
  ...
  const res = await fetch(url.toString(), {
    headers: authHeaders(),
    // Show/episode data changes rarely; cache for an hour at the edge.
...
export function getShowDetails(showId) {
  return tmdbFetch(`/tv/${showId}`, { append_to_response: "credits,videos,aggregate_credits" });
}
...
export function getShowImages(showId, originalLanguage, extraLanguages = []) {
  ...
  return tmdbFetch(`/tv/${showId}/images`, { include_image_language: [...langs].join(",") });
}
...
export function getPersonDetails(personId) {
  return tmdbFetch(`/person/${personId}`, { append_to_response: "combined_credits" });
}
...
export function getMovieImages(movieId, originalLanguage, extraLanguages = []) {
  ...
  if (originalLanguage) langs.add(originalLanguage);
  for (const l of extraLanguages) if (l) langs.add(l);
  return tmdbFetch(`/movie/${movieId}/images`, { include_image_language: [...langs].join(",") });
}
