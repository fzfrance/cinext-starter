...
// userMovies: [{ movieId, status, favorite, addedAt, updatedAt }] — lib/userMovies.js's
//   getUserMovies, reshaped the same way userShows is by the caller.
// movieRatings: lib/movieRatings.js's getAllMovieRatingsForUser return value
//
// Every event now carries mediaType: "tv" | "movie" — the TV events below
// are unchanged in every other way (still keyed on showId), movies are
// keyed on movieId instead. Movies get only "added"/"completed"/"rated" —
// no "watched"/"rewatched" (no episodes to watch one at a time) and no
// resolveShowStatus-style live derivation (a movie's status is always
// exactly what the user picked, see lib/userMovies.js, so raw
// status === "completed" is already the whole rule — no progress-based
// fallback branch needed the way shows have one). No addedToCollection
// for movies in this pass either — out of scope of the request this was
// built against (just added/completed/status).
export function buildActivityEvents({ userShows, episodeWatches, seasonRatings, collectionAdditions = [], progressByShowId = {}, userMovies = [], movieRatings = [] }) {
  const events = [];
  ...
  for (const s of userShows) {
    events.push({ type: "added", mediaType: "tv", showId: s.showId, timestamp: new Date(s.addedAt) });
    ...
    if (s.status === "completed") {
      ...
      events.push({ type: "completed", mediaType: "tv", showId: s.showId, timestamp: new Date(s.updatedAt) });
    } else {
      ...
      if (progress) {
        ...
        if (resolved === "completed" && lastWatchedAtByShow[s.showId]) {
          events.push({ type: "completed", mediaType: "tv", showId: s.showId, timestamp: lastWatchedAtByShow[s.showId] });
        }
      }
    }
  }
  ...
  // Movies — added/completed only (no episode-level watched/rewatched,
  // no resolveShowStatus-style live derivation; see the header comment
  // above for the full reasoning).
  for (const m of userMovies) {
    events.push({ type: "added", mediaType: "movie", movieId: m.movieId, timestamp: new Date(m.addedAt) });
    if (m.status === "completed") {
      events.push({ type: "completed", mediaType: "movie", movieId: m.movieId, timestamp: new Date(m.updatedAt) });
    }
  }
