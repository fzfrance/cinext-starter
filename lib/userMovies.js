...
const userMoviesInFlight = new Map();
const USER_MOVIES_PAGE_SIZE = 1000;

async function loadUserMovies(userId) {
  const rows = [];
  // PostgREST limits a response to 1,000 rows by default. A large import can
  // legitimately exceed that, so read the complete library in stable pages.
  // Without this, newer watchlist entries disappear even though they were
  // saved correctly in user_movies.
  for (let from = 0; ; from += USER_MOVIES_PAGE_SIZE) {
    const { data, error } = await supabase
      .from("user_movies")
      .select("tmdb_movie_id, status, favorite, created_at, updated_at")
      .eq("user_id", userId)
      .order("tmdb_movie_id", { ascending: true })
      .range(from, from + USER_MOVIES_PAGE_SIZE - 1);
    if (error) throw error;
    rows.push(...data);
    if (data.length < USER_MOVIES_PAGE_SIZE) break;
  }
...
// Deduplicate simultaneous provider/page loads without retaining settled
// personal data. The next call after this request finishes always reaches
// Supabase again.
export function getUserMovies(userId) {
  const existing = userMoviesInFlight.get(userId);
  if (existing) return existing;

  const request = loadUserMovies(userId);
  userMoviesInFlight.set(userId, request);
  request.then(
    () => { if (userMoviesInFlight.get(userId) === request) userMoviesInFlight.delete(userId); },
    () => { if (userMoviesInFlight.get(userId) === request) userMoviesInFlight.delete(userId); }
  );
  return request;
}
...
const WATCHED_MOVIES_PAGE_SIZE = 1000;

const mapWatchedMovieRow = (r) => ({
...
async function getPaginatedWatchedMovies(userId, applyScope) {
  const rows = [];
  for (let from = 0; ; from += WATCHED_MOVIES_PAGE_SIZE) {
    let query = supabase
      .from("user_movies")
    ...
    query = applyScope(query)
      .order("tmdb_movie_id", { ascending: true })
      .range(from, from + WATCHED_MOVIES_PAGE_SIZE - 1);
    const { data, error } = await query;
    if (error) throw error;
    rows.push(...data);
    if (data.length < WATCHED_MOVIES_PAGE_SIZE) break;
  }
  return rows.map(mapWatchedMovieRow);
}
...
// by construction — same as TV's own behavior. This used to assume a
// movie library would never reach PostgREST's response cap, but imports
// can legitimately put hundreds of films in a
// single historical year, so use the same explicit pagination as TV watch
// events. A stable id order prevents gaps/duplicates between pages.
export async function getUserMoviesWatchedInYear(userId, year) {
  return getPaginatedWatchedMovies(userId, (query) => query.eq("watched_year", year));
}
...
// aren't silently dropped. Explicit pagination is required here: a user
// can now import a library larger than PostgREST's 1000-row response cap.
export async function getAllUserMoviesWatched(userId) {
  return getPaginatedWatchedMovies(userId, (query) => query.not("watched_year", "is", null));
}
