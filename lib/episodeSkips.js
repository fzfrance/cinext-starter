...
async function setEpisodeSkippedImpl(userId, tmdbShowId, seasonNumber, episodeNumber, skipped) {
  if (skipped) {
...
export async function deleteAllEpisodeSkipsForShow(userId, tmdbShowId) {
  ...
  const { error } = await supabase.from("episode_skips").delete().eq("user_id", userId).eq("tmdb_show_id", showId);
  if (error) throw error;
}

export function setEpisodeSkipped(...args) {
  return mutateShow(args[0], args[1], () => setEpisodeSkippedImpl(...args));
}
