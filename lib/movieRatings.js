...
export async function getAllMovieRatingsForUser(userId) {
  const { data, error } = await supabase
    .from("movie_ratings")
    .select("tmdb_movie_id, rating, mood, favorite_character_id, favorite_character_name, review_text, share_id, created_at, updated_at")
    .eq("user_id", userId);
...
export async function getMovieRating(userId, tmdbMovieId) {
  const { data, error } = await supabase
    .from("movie_ratings")
    .select("rating, mood, favorite_character_id, favorite_character_name, review_text, share_id, updated_at")
    .eq("user_id", userId)
...
export async function saveMovieRating(userId, tmdbMovieId, { rating, mood, characterId, characterName, text }) {
...
export async function getMovieRatingByShareId(shareId) {
  const { data, error } = await supabase
    .from("movie_ratings")
    .select("tmdb_movie_id, rating, mood, favorite_character_id, favorite_character_name, review_text, updated_at")
    .eq("share_id", shareId)
    .maybeSingle();
  if (error) throw error;
  if (!data) return null;
