...
// ---------------------------------------------------------------------------
// Shared "decorate any show/movie with this user's real library status, and
// let them change it from a StatusMenu" logic — extracted so the global
// Search screen (app/search/SearchClient.jsx) doesn't need its own
// re-implementation of Explore's own status-map machinery
...
//
// Mixed movies + TV shows (movies-as-content-type plan): items are shaped
// {id, mediaType, ...} and every internal map is keyed by the composite
// mediaKey (movie and TV ids aren't globally unique — two unrelated
// titles can share a numeric id). Since this hook currently has exactly
// one caller (SearchClient.jsx), it's edited in place rather than forked
// the way Show Detail vs. Movie Detail were.
// ---------------------------------------------------------------------------
...
export function useLibraryStatus(source) {
  ...
  // resolved from real released/watched episode counts for shows (movies
  // have no progress to resolve — their stored status IS the displayed
  // one), same as every other screen (lib/statusResolver.js).
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const [byShow, byMovie] = await Promise.all([getUserShows(user.id), getUserMovies(user.id)]);
      if (cancelled) return;
      const map = {};
  ...
  // clearing real watch history. Movies have no watch-progress concept at
  // all (lib/userMovies.js), so this is never called for a movie item.
  ...
  const selectStatus = async (item, statusId) => {
    ...
    if (statusId === "remove") {
      ...
      removeFn(user.id, item.id, `${source}:selectStatus:remove`)
        .then(() => {
          setStatusMap((prev) => { const next = { ...prev }; delete next[key]; return next; });
    ...
    if (statusId === "watchlist") {
      ...
      setStatusMap((prev) => ({ ...prev, [key]: "watchlist" }));
      setResolvedStatusMap((prev) => ({ ...prev, [key]: "watchlist" }));
      const writePromise = isMovie
        ? setMovieStatus(user.id, item.id, "watchlist", `${source}:selectStatus:watchlist`)
        : setWatchlistAndClearProgress(user.id, item.id, `${source}:selectStatus:watchlist-confirm`);
      writePromise.catch(console.error);
      return;
    }
    setStatusMap((prev) => ({ ...prev, [key]: statusId }));
    setResolvedStatusMap((prev) => ({ ...prev, [key]: statusId }));
    if (isMovie) {
      setMovieStatus(user.id, item.id, statusId, `${source}:selectStatus`).catch(console.error);
    } else {
  ...
  return { statusMap, resolvedStatusMap, selectStatus };
}
