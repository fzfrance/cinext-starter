...
## Confirmed Gotham diagnosis and status-button clarification

A read-only query was authorized for the exact supplied profile handle in the supplied Supabase project. The handle resolved to France; every subsequent query filtered that profile's user_id AND Gotham (TMDB 60708). Current counts are zero for episode_watches, episode_skips, season_reviews, season_ratings, and user_shows. No production records were changed.

A concrete callback error was then found in BOTH ShowDetailClient variants: `setFavorite(false)` references a nonexistent setter (favorite now comes from FavoritesProvider), and `setWatchedShowIds` is also undefined. The first exception occurs after database deletion but before setSeasons, leaving stale episode progress on screen. These obsolete setters are removed; FavoritesProvider already refreshes on the shared invalidation event. New tests execute the actual selectStatus('remove') handlers with five populated seasons and cover both successful and failed deletion.

Show Detail now refreshes its read-only watch, skip, library, and rating state on foreground return, bfcache restoration, or account-scoped history invalidation. Snapshot application clears fields for absent database rows instead of only adding watched/skipped flags. Two additional tests cover retained middle-season state and obsolete ratings. These changes address observed stale-state paths; the exact historical sequence of the first/last-season symptom was not captured in a browser.

The user clarified that show/movie status buttons and Home's watch button should be white. Those controls retain their original white treatment; the separately requested amber rating actions remain amber.
