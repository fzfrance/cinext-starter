...
import StarInput from "@/components/mobile-original/ui/StarInput";
import PosterArt from "@/components/mobile-original/ui/PosterArt";
import MiniDatePicker from "@/components/mobile-original/ui/MiniDatePicker";
import { tmdbImage } from "@/lib/tmdb";
import { SEASON_MOOD_LIST, moodIdsFromField, moodMetasFromField } from "@/components/mobile-original/SeasonBanner";
      ...
      {justSaved && (
        <div className="absolute inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(0,0,0,0.6)" }}>
          <div className="w-full rounded-t-3xl flex flex-col items-center text-center flex-shrink-0" style={{ padding: "40px 28px 28px", background: "#161210", border: `1px solid ${t.glassBorder}`, borderBottom: "none", boxShadow: "0 -20px 60px rgba(0,0,0,0.6)" }}>
            ...
            <button onClick={() => setJustSaved(false)} className="w-full rounded-full active:scale-95 transition" style={{ marginTop: 24, padding: 14, background: accent }}><span style={{ fontSize: 14.5, fontWeight: 700, color: "#111" }}>Done</span></button>
          </div>
        </div>
      )}
    </div>
